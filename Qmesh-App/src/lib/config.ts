...
export async function probeBackend(
  ...
  // Keyed off the address, not `available`: a mode blocked because the selected
  // model is too big for it (blockedBy === "model") is still running, and
  // reporting it down would be a false claim about the engine.
  if (!p.baseUrl) return { reach: "down", backend: "unknown" };
